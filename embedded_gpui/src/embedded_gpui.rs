...
//!
//! - the **schema layer** (this file, always compiled): specs, messages, receipts, and
//!   [`SharedCaller`] — the typed veneer over the dynamic shared-entity wire, where
//!   everything is `(entity_id, method: string, payload: bytes)`;
//! - the **host runtime** ([`PluginHost`] and friends; native targets only): wasmtime
//!   glue, display-list replay, and host-side shared entities;
...
/// `embedded_gpui_util::Revocable` work identically on either side of the boundary.
