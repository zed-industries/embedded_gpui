//! A plugin component that embeds QuickJS and runs JavaScript plugins against the object
//! model. It is an ordinary plugin: its root object answers `load(source)` itself and
//! forwards every other method to the script's `plugin.root`; remotes reach JS as
//! proxies whose methods return promises; UI is a data tree the script renders and this
//! crate turns into GPUI elements on a host surface.
//!
...
use embedded_gpui::{
    Methods, Opaque, Payload, Plugin, Ref, Remote, Shared, WILDCARD_METHOD, open_view,
    register_plugin, registry, share_root,
};
