//! Proc macros for `gpui_embedded_shared`. See [`macro@shared_interface`].
...
fn expand(
    ...
    let message_items = methods.iter().map(|method| {
        ...
        quote! {
            #[derive(Clone, Debug, gpui_embedded_shared::serde::Serialize, gpui_embedded_shared::serde::Deserialize)]
            #[serde(crate = "gpui_embedded_shared::serde")]
            #vis struct #message_ident {
                #(pub #field_names: #field_types,)*
            }

            impl gpui_embedded_shared::SharedMessage for #message_ident {
                type Spec = #spec;
    ...
    let caller_methods = methods.iter().map(|method| {
        ...
        quote! {
            fn #ident(
                ...
                cx: &mut gpui_embedded_shared::gpui::App,
            ) -> gpui_embedded_shared::CallReceipt<#response> {
    ...
    let registrations = methods.iter().map(|method| {
        ...
        quote! {
            methods.on_raw(#method_name, |entity, _method, payload, cx| {
                ...
                gpui_embedded_shared::encode(&response)
            });
        }
    });
    ...
    let trait_doc = format!(
        "Typed calls to a shared `{spec}` entity, for any holder of a capability to one \
         ([`SharedCaller`](gpui_embedded_shared::SharedCaller)): guest `Remote` and host \
         `HostRemote` alike."
    );
    ...
    Ok(quote! {
        ...
        impl gpui_embedded_shared::SharedSpec for #spec {
        ...
        #[doc = #trait_doc]
        #vis trait #caller_ident: gpui_embedded_shared::SharedCaller<#spec> {
            #(#caller_methods)*
        }

        impl<C: gpui_embedded_shared::SharedCaller<#spec>> #caller_ident for C {}

        #[doc = #register_doc]
        #vis fn #register_ident<T>(methods: &mut gpui_embedded_shared::Methods<#spec, T>)
        where
