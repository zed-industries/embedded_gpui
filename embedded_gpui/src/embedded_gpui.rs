...
/// Identifies a kind of shared entity by a stable wire name. Generated by
/// [`shared_interface`] as a struct with the same name as the interface, so a handle to
/// a shared counter reads `Remote<CounterApi>`.
pub trait SharedSpec: 'static {
    /// Stable type name checked when a remote binds to an announcement.
