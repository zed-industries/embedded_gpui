...
///   `#[shared_home] impl CounterApi for MyEntity { ... }`;
...
///   pointer per method, which is how `#[shared_home]` type-checks a home
...
/// entity, and the attribute implements `SharedHome<CounterApi>` by registering each
...
#[proc_macro_attribute]
pub fn shared_home(_attr: TokenStream, item: TokenStream) -> TokenStream {
...
fn expand_interface(
    ...
    // The checked registration functions #[shared_home] compiles against: the function
    ...
    let spec_doc = format!(
        "\n\nThe `{spec_ident}` shared interface (wire name `{}`): hold one as \
         ...
         one with `#[shared_home] impl {spec_ident} for ...`.",
        type_name.value()
    );
...
fn expand_home(mut item_impl: ItemImpl) -> syn::Result<proc_macro2::TokenStream> {
    let Some((None, spec_path, _for_token)) = item_impl.trait_.clone() else {
        return Err(syn::Error::new(
            item_impl.span(),
            "#[shared_home] goes on `impl SomeInterface for SomeEntity { ... }`",
        ));
    };
    ...
    Ok(quote! {
        ...
        impl #impl_generics embedded_gpui::SharedHome<#spec_path> for #self_ty #where_clause {
